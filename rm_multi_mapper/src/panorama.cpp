...
Eigen::MatrixXf matrix2eigen(rm_multi_mapper::Matrix m1, Eigen::MatrixXf eigen) {

    for(int i=0;i<m1.matrix.size();i++)
    {
        for(int j=0;j<m1.matrix[0].matrow.size();j++)
        {
            eigen(i,j) = m1.matrix[i].matrow[j];
...
int main(int argc, char **argv) {
    ...
    for(int i=0; i<overlapping_keyframes.size()/2;i++)
    {
    ...
    for(int i=overlapping_keyframes.size()/2; i<overlapping_keyframes.size();i++)
    {
        rm_multi_mapper::KeyframePair keyframe;
        keyframe.first = overlapping_keyframes[i].first;
        keyframe.second = overlapping_keyframes[i].second;
    ...
    if (finished_before_timeout1 && finished_before_timeout2)
    {
    	...
    	rm_multi_mapper::Matrix rosJte = ac1.getResult()->Jte;
    	rm_multi_mapper::Matrix rosJtJ = ac1.getResult()->JtJ;
    	
    	rm_multi_mapper::Matrix rosJte2 = ac2.getResult()->Jte;
    	rm_multi_mapper::Matrix rosJtJ2 = ac2.getResult()->JtJ;
    	
        Jte = matrix2eigen(rosJte, Jte);
        JtJ = matrix2eigen(rosJtJ, JtJ);
        
        Jte2 = matrix2eigen(rosJte2, Jte2);
        JtJ2 = matrix2eigen(rosJtJ2, JtJ2);
        
        JtJ += JtJ2;
        Jte += Jte2;
        
        std::cout<<Jte(0, 0)<<std::endl;
        std::cout<<JtJ(0, 0)<<std::endl;
    }
    else 
    {
