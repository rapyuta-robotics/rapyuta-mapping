...
static void warpImage(const Mat& image, const Mat& depth, const Mat& Rt,
		const Mat& cameraMatrix, const Mat& distCoeff, Mat& warpedImage) {
	...
	projectPoints(transformedCloud.reshape(3, 1), Mat::eye(3, 3, CV_64FC1),
			Mat::zeros(3, 1, CV_64FC1), cameraMatrix, distCoeff, points2d);
	...
	for (int y = 0; y < image.rows; y++) {
		for (int x = 0; x < image.cols; x++) {
			...
			if (!cvIsNaN(cloud.at<Point3f>(y, x).z)
					&& cloud.at<Point3f>(y, x).z > 0 && rect.contains(p2d)
					&& zBuffer.at<float>(p2d) > p3d.z) {
				warpedImage.at<ImageElemType>(p2d) = image.at<ImageElemType>(y,
						x);
				zBuffer.at<float>(p2d) = p3d.z;
			}
		}
...
int main(int argc, char** argv) {
	float vals[] = { 525., 0., 3.1950000000000000e+02, 0., 525.,
	...
	if (argc != 5 && argc != 6) {
		cout << "Format: image0 depth0 image1 depth1 [transformationType]"
				<< endl;
	...
	tm.start();
	bool isFound = cv::RGBDOdometry(Rt, Mat(), grayImage0, depthFlt0, Mat(),
			grayImage1, depthFlt1, Mat(), cameraMatrix, minDepth, maxDepth,
			maxDepthDiff, iterCounts, minGradMagnitudes, transformationType);
	tm.stop();
	...
	if (!isFound) {
		cout << "Rigid body motion cann't be estimated for given RGBD data."
				<< endl;
		return -1;
	}

	Mat warpedImage0;
	warpImage<Point3_<uchar> >(colorImage0, depthFlt0, Rt, cameraMatrix,
			distCoeff, warpedImage0);
