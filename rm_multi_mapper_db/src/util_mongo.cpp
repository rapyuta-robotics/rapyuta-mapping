...
void util_mongo::add_keyframe(int robot_id, const color_keyframe::Ptr & k) {
	...
	query2.appendBinData("rgb", rgb_data.size(), BinDataGeneral, (char*) rgb_data.data());

	cv::imencode(".png", k->get_d(0), depth_data);

	query2.appendBinData("depth", depth_data.size(), BinDataGeneral, (char*) depth_data.data());
...
void util_mongo::add_keypoints(const color_keyframe::Ptr & k) {
	...
	query.appendBinData("descriptors", descriptors.cols * descriptors.rows * sizeof(float),
						BinDataGeneral, descriptors.data);
	query.appendBinData("keypoints", keypoints3d.points.size() * sizeof(pcl::PointXYZ),
						BinDataGeneral, keypoints3d.points.data());

	conn.update("mapping.keyframe",
		       BSON("_id" << (long long)k->get_id()),
		       BSON("$set" << query.obj()));
	string e = conn.getLastError();
	if( !e.empty() ) {
...
boost::shared_ptr<keyframe_map> util_mongo::get_robot_map(int robot_id) {
	...
	auto_ptr<DBClientCursor> cursor =
	    conn.query("mapping.keyframe", QUERY("map_id" << map_id));
...
void util_mongo::load_measurements(long keyframe_id,
	...
	auto_ptr<DBClientCursor> cursor =
	    conn.query("mapping.measurement", QUERY("one" << (long long)keyframe_id));
...
void util_mongo::merge_map(int old_map_id, int new_map_id){
	conn.update("mapping.robot",
		       BSON("map_id" << (int)old_map_id),
